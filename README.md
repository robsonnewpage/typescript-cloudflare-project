...
| **D1** (SQLite) | `meeting-intelligence-db` | `DB` | The relational core — `meetings`, `facts`, `threads`. Schema and migrations managed with [Drizzle ORM](https://orm.drizzle.team/) (`src/db/`, `migrations/`). |
...
A standard Next.js App Router project at the repo root — pages, Server Actions, API routes and the Worker entry all live together, since this is one deployable Worker.

```
src/app/             routes: pages, Server Actions, route handlers
src/components/      client and server UI components
src/lib/             data layer, validation, cron job, R2 signing, WebVTT parser
src/durable-objects/ ThreadArbiterDO
src/db/              Drizzle schema, D1 client, seed SQL
migrations/          D1 migrations generated by Drizzle
custom-worker.ts     Worker entry: OpenNext fetch + Durable Object + scheduled()
wrangler.jsonc       every Cloudflare binding
Makefile             thin wrapper around the npm scripts
```
...
make cf-typegen    # generate cloudflare-env.d.ts (gitignored) — needed before the first build
...
`.dev.vars` holds local secrets (`ADMIN_TOKEN`, `R2_TRANSCRIPTS_ACCOUNT_ID`, `R2_TRANSCRIPTS_ACCESS_KEY_ID`, `R2_TRANSCRIPTS_SECRET_ACCESS_KEY`; see `src/env.d.ts`) — copy `.dev.vars.example` if present, or ask whoever set up the Cloudflare account for values. See `wrangler.jsonc` for the full binding list. Run `make cf-typegen` again after changing its bindings.
